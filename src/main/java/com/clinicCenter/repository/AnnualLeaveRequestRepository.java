...
public interface AnnualLeaveRequestRepository extends JpaRepository<AnnualLeaveRequest,Long> {
    ...
    @Query( value = "select * from db.annual_leave_request a where a.flag='vacation' and a.approved = false and a.user_id in " +
    ...
    @Query(value = " select * from db.annual_leave_request as a where a.flag='absence' and approved = false and a.user_id in " +
                    "(select u.id from db.users u where u.clinic_id = " +
                    "(select uu.clinic_id from db.users uu where uu.id = :id))" , nativeQuery = true)
